...
type HanhwalApplication = ApplicationForm & {
  id: string;
...
type ApplicationsApiResponse = {
  ...
  applications?: HanhwalApplication[];
  error?: string;
};
...
const initialNoticeForms: Record<Language, NoticeForm> = {
  ...
  en: {
    title: "HANHWAL Regular Gathering",
    dateTime: "",
    location: "",
...
const defaultApplicationTypes: Array<{
...
}> = [
  {
    ...
    labels: {
      ko: {
        title: "정기 국궁 연습 신청",
        description: "한활 정기 국궁 연습 참여 신청"
      },
      en: {
        title: "Regular Archery Practice Application",
        description: "Apply for Hanhwal regular archery practice"
      }
    }
  },
  {
    ...
    labels: {
      ko: {
        title: "한활 수련회 신청",
        description: "한활 국궁 수련회 참여 신청"
      },
      en: {
        title: "Hanhwal Training Camp Application",
        description: "Apply for the Hanhwal training camp"
      }
    }
  },
  {
    ...
    labels: {
      ko: {
        title: "국궁 특별 행사 신청",
        description: "한활 국궁 특별 행사 참여 신청"
      },
      en: {
        title: "Traditional Archery Event Application",
        description: "Apply for a Hanhwal traditional archery event"
      }
    }
  },
  {
    ...
    labels: {
      ko: {
        title: "학기 첫 연습 신청",
        description: "한활 학기 첫 연습 참여 신청"
      },
      en: {
        title: "Semester Opening Practice Application",
        description: "Apply for the Hanhwal semester opening practice"
      }
    }
  },
  {
    ...
    labels: {
      ko: {
        title: "학기 마지막 연습 신청",
        description: "한활 학기 마지막 연습 참여 신청"
      },
      en: {
        title: "Semester Closing Practice Application",
        description: "Apply for the Hanhwal semester closing practice"
      }
    }
  },
  {
    ...
    labels: {
      ko: {
        title: "초보 국궁 교실 신청",
        description: "한활 초보 국궁 교실 참여 신청"
      },
      en: {
        title: "Beginner Archery Class Application",
        description: "Apply for the Hanhwal beginner archery class"
      }
    }
  }
];
...
const copy = {
  ko: {
    ...
    readOnlyDescription:
      "일반 회원은 공개된 한활 회원 현황과 활동 내용을 확인할 수 있습니다. 붙여넣기, 조 편성, 공지문 생성은 관리자 이상으로 로그인한 경우에만 표시됩니다.",
  ...
  en: {
    ...
    members: "Members",
    registeredMembers: "Registered HANHWAL members",
    ...
    readOnlyDescription:
      "General members can view the public HANHWAL member status and activity records. Pasting data, making teams, and generating notices appear only for admins and higher roles.",
    ...
    emptyMembersDescription: "Log in as an admin and paste the HANHWAL member list.",
    teamMaker: "Team maker",
    ...
    noticePrefix: "[HANHWAL Notice]",
    defaultNoticeTitle: "HANHWAL Activity",
    dateLabel: "Date/Time",
...
function parseMembersFromPaste(value: string): HanhwalMember[] {
  const lines = value
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  return dataLines
    .map((line, index) => {
      ...
      return {
        id: `hanhwal-member-${Date.now()}-${index}-${name}`,
        name,
    ...
    .filter((member): member is HanhwalMember => Boolean(member));
}

function memberScore(member: HanhwalMember) {
...
function makeTeams(members: HanhwalMember[], teamSize: number, language: Language) {
  const size = Math.max(1, teamSize);
  const teamCount = Math.max(1, Math.ceil(members.length / size));
  const teams: HanhwalTeam[] = Array.from({ length: teamCount }, (_, index) => ({
    id: `hanhwal-team-${index + 1}`,
...
function formatTeamName(team: HanhwalTeam, index: number, language: Language) {
  return team.name.match(/^\d+조$/) || team.name.match(/^Team\s+\d+$/i)
    ? teamName(index, language)
    : team.name;
}

function buildTeamText(teams: HanhwalTeam[], language: Language) {
  return teams
    .map(
      (team, index) =>
...
function readStoredTeams() {
  try {
    const raw = window.localStorage.getItem(adminStorageKeys.hanhwalActivityTeams);
    return raw ? (JSON.parse(raw) as HanhwalTeam[]) : [];
  } catch {
    return [];
  }
}
...
function readStoredTeamText() {
  try {
    return window.localStorage.getItem(adminStorageKeys.hanhwalActivityTeamsText) ?? "";
  } catch {
    return "";
  }
}
...
function readStoredNotice() {
  try {
    return window.localStorage.getItem(adminStorageKeys.hanhwalActivityNotice) ?? "";
  } catch {
    return "";
  }
}
...
export function HanhwalActivityPanel() {
  ...
  const [applications, setApplications] = useState<HanhwalApplication[]>([]);
  ...
  const [teams, setTeams] = useState<HanhwalTeam[]>([]);
    ...
    const loadApplications = async () => {
      ...
      try {
        const [applicationsResponse, statusesResponse, catalogResponse] =
          await Promise.all([
            fetch("/api/hanhwal/applications", {
              signal: controller.signal
            }),
            fetch("/api/hanhwal/activity-statuses", {
              signal: controller.signal
            }),
            fetch("/api/hanhwal/activity-catalog", {
              signal: controller.signal
            })
          ]);
  ...
  const changeLanguage = (nextLanguage: Language) => {
    ...
    setLanguage((currentLanguage) => {
      ...
      setNotice("");
      setCopyMessage("");
      window.localStorage.setItem(adminStorageKeys.hanhwalActivityLanguage, nextLanguage);
      window.localStorage.removeItem(adminStorageKeys.hanhwalActivityNotice);
      return nextLanguage;
    });
  };
  ...
  const createManagedActivity = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/hanhwal/activity-catalog", {
        method: "POST",
  ...
  const archiveManagedActivity = async (activityId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/hanhwal/activity-catalog?id=${encodeURIComponent(activityId)}`,
        { method: "DELETE" }
      );
      const data = (await response.json()) as ActivityCatalogApiResponse;
  ...
  const submitApplication = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/hanhwal/applications", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const saveTeams = (nextTeams: HanhwalTeam[]) => {
    const nextTeamText = buildTeamText(nextTeams, language);
    window.localStorage.setItem(adminStorageKeys.hanhwalActivityTeams, JSON.stringify(nextTeams));
    window.localStorage.setItem(adminStorageKeys.hanhwalActivityTeamsText, nextTeamText);
    setTeams(nextTeams);
  ...
  const buildTeamsFromCurrentList = () => {
    ...
    const applicantMembers: HanhwalMember[] = selectedApplications.map((application, index) => ({
      id: application.id,
  ...
  const generateNotice = () => {
    ...
    window.localStorage.setItem(adminStorageKeys.hanhwalActivityNotice, nextNotice);
  };
  ...
  const updateTeamText = (value: string) => {
    setTeamText(value);
    window.localStorage.setItem(adminStorageKeys.hanhwalActivityTeamsText, value);
  };
  ...
  const resetApplicants = async () => {
    ...
    try {
      const response = await fetch(
        `/api/hanhwal/applications?activity_id=${encodeURIComponent(activeApplicationType)}`,
        {
          method: "DELETE"
        }
      );
