...
type HanhwalMemberRegistration = {
  id: string;
  googleEmail: string;
  googleName: string;
...
type RegistrationListResponse = {
  ...
  error?: string;
  registrations?: HanhwalMemberRegistration[];
  updatedCount?: number;
  updatedRegistrations?: HanhwalMemberRegistration[];
};
...
function statusLabel(registration: HanhwalMemberRegistration, language: "en" | "ko") {
...
function toDrafts(nextRegistrations: HanhwalMemberRegistration[]) {
...
export function HanhwalMemberRegistrationManagementPanel() {
  const { language } = useLanguage();
  const [registrations, setRegistrations] = useState<HanhwalMemberRegistration[]>([]);
  ...
  const applyRegistrations = (nextRegistrations: HanhwalMemberRegistration[]) => {
    setRegistrations(nextRegistrations);
    setDrafts(toDrafts(nextRegistrations));
  };

  const mergeUpdatedRegistrations = (updatedRegistrations: HanhwalMemberRegistration[]) => {
  ...
  useEffect(() => {
    ...
    fetch("/api/hanhwal/member-registrations")
      .then((response) =>
      ...
      .then(({ data, response }) => {
        ...
        if (!response.ok) {
          throw new Error(data.error || "HANHWAL member registrations could not be loaded.");
        }
      ...
      .catch((loadError) => {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "HANHWAL member registrations could not be loaded."
        );
      })
      .finally(() => {
        if (active) {
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch("/api/hanhwal/member-registrations", {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || "HANHWAL member registrations could not be saved.");
      }
      ...
      setMessage(
        language === "ko"
          ...
          : `${data.updatedCount ?? 0} HANHWAL member registration records saved.`
      );
    } catch (saveError) {
      setError(
        saveError instanceof Error
          ? saveError.message
          : "HANHWAL member registrations could not be saved."
      );
    } finally {
      setSaving(false);
    }
  ...
  const deleteRegistration = async (registration: HanhwalMemberRegistration) => {
    const confirmed = window.confirm(
      language === "ko"
        ? `${registration.fullName || registration.googleEmail}님의 한활 가입 신청과 한활 자체 권한을 초기화하시겠습니까? 사이트 계정과 다른 K_LINE 데이터는 유지됩니다.`
        : `Reset ${registration.fullName || registration.googleEmail}'s HANHWAL registration and HANHWAL-specific permission data? Their site account and other K_LINE data will remain.`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "HANHWAL member registration could not be reset.");
      }

      applyRegistrations(data.registrations ?? []);
      setMessage(
        language === "ko"
          ? "한활 가입 신청과 한활 자체 권한을 초기화했습니다."
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "HANHWAL member registration could not be reset."
      );
    } finally {
      setDeletingId("");
    }
  ...
  if (loading) {
    return (
      <div className="paper-panel flex items-center gap-3 p-6 text-sm font-semibold text-ink/62">
        ...
        <I18nText en="Loading HANHWAL new member registrations..." ko="한활 신규회원 등록을 불러오는 중..." />
      </div>
    );
  }
  ...
  return (
    <section className="paper-panel overflow-hidden">
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-ink/68">
              <I18nText
                en="Check membership fee payment, save, and the selected Google account becomes an official HANHWAL member with access to HANHWAL OFFICIAL."
                ko="회비 납부 여부를 체크하고 저장하면 해당 Google 계정이 한활 정식회원으로 승인되어 한활 OFFICIAL 접근 권한을 얻습니다."
              />
            </p>
          </div>
      ...
      {registrations.length > 0 ? (
        ...
        <div className="p-8 text-sm leading-7 text-ink/62">
          <I18nText
            en="No HANHWAL member registrations have been submitted yet."
            ko="아직 한활 신규회원 등록이 없습니다."
          />
        </div>
      )}
