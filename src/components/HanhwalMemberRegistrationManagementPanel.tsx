...
export function HanhwalMemberRegistrationManagementPanel() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Hanhwal member registrations could not be saved.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <section className="paper-panel overflow-hidden">
          ...
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-ink/68">
              <I18nText
                ...
                ko="회비 납부 여부를 체크하고 저장하면 해당 Google 계정이 Hanhwal 정식회원으로 승인되어 Hanhwal OFFICIAL 접근 권한을 얻습니다."
              />
            </p>
          </div>
      ...
      {registrations.length > 0 ? (
        ...
        <div className="p-8 text-sm leading-7 text-ink/62">
          <I18nText
            en="No K_LINE internal Hanhwal member registration has been submitted yet."
            ko="아직 K_LINE 내부 Hanhwal 신규회원 등록이 없습니다."
          />
        </div>
      )}
