...
export function HanhwalAlumniInquiryForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/hanhwal-alumni/inquiries", {
        method: "POST",
    ...
    <form onSubmit={submit} className="paper-panel grid gap-4 p-5 md:p-8">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-semibold text-ink">
          ...
          <select className="form-field" value={form.currentStatus} onChange={(event) => update("currentStatus", event.target.value)}>
            <option value="current HANHWAL member">Current HANHWAL member</option>
            <option value="previous HANHWAL member">Previous HANHWAL member</option>
      ...
      {error ? (
        <p className="text-sm font-semibold text-red-700">
          {error}{" "}
          <Link href="/login?callbackUrl=/hanhwal-alumni/activity-inquiry" className="underline">
            Log in
          </Link>
        </p>
      ) : null}
...
export function HanhwalAlumniStatusList() {
  ...
  useEffect(() => {
    void Promise.all([
      fetch("/api/hanhwal-alumni/inquiries").then((response) => response.json()).catch(() => ({})),
      fetch("/api/hanhwal-alumni/rejoin-requests").then((response) => response.json()).catch(() => ({}))
