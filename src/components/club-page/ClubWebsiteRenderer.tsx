...
export function ClubWebsiteRenderer({ document }: { document: ClubDocument }) {
  const theme =
    THEME_PRESETS.find((item) => item.id === document.themeId) ||
    THEME_PRESETS[0];
  const variables = {
    "--club-primary": theme.primary,
    "--club-on-primary":
      theme.id === "warm" || theme.id === "nature" ? "#111111" : "#ffffff",
  ...
  return (
    <div className={styles.website} style={variables}>
      {document.sections.map((section) => {
        // The discriminated schema is validated before rendering. Registry keys retain that pairing.
        const Renderer = SECTION_RENDERERS[section.type] as ComponentType<{
          data: ClubSection["data"];
        }>;
