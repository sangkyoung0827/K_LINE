...
export function HanhwalDonationPanel({ canEdit }: { canEdit: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/hanhwal/fund", { cache: "no-store" })
      .then(async (response) => {
        if (!response.ok) {
          throw new Error("HANHWAL fund request failed");
        }
  ...
  const saveFund = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/hanhwal/fund", {
        method: "PATCH",
      ...
      if (!response.ok) {
        throw new Error("HANHWAL fund update failed");
      }
    ...
    <div className={`grid gap-6 ${canEdit ? "lg:grid-cols-2" : "max-w-2xl"}`}>
      {canEdit ? (
        <form onSubmit={saveFund} className="paper-panel p-6 md:p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-ink/64">
            <I18nText
              en="Administrators can enter the current HANHWAL balance manually."
              ko="관리자 이상 권한에서 현재 한활 자금을 직접 입력할 수 있습니다."
            />
          </p>
          <label className="mt-6 block text-sm font-semibold text-ink" htmlFor="hanhwal-fund-amount">
            <I18nText en="Current fund (KRW)" ko="현재 자금 (원)" />
          </label>
          <input
            id="hanhwal-fund-amount"
      ...
      <section className="paper-panel p-6 md:p-8" aria-live="polite">
        ...
        <p className="mt-6 text-sm font-semibold uppercase text-brass">
          <I18nText en="HANHWAL balance" ko="한활 잔액" />
        </p>
        <h2 className="mt-3 font-serif text-3xl font-semibold text-ink">
