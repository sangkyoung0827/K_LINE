...
function readDonationPledges() {
  try {
    const raw = window.localStorage.getItem(adminStorageKeys.hanhwalDonationPledges);
...
export function HanhwalDonationPanel() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.localStorage.getItem(adminStorageKeys.hanhwalBankSnapshot);
      if (!raw) {
        return;
      }
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    window.localStorage.setItem(adminStorageKeys.hanhwalDonationPledges, JSON.stringify(nextDonations));
  ...
  const saveBankDisplay = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    window.localStorage.setItem(
      adminStorageKeys.hanhwalBankSnapshot,
      JSON.stringify({
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="paper-panel p-6 md:p-8">
          ...
          <h2 className="mt-3 font-serif text-4xl font-semibold text-ink">
            <I18nText en="Hanhwal Donations and Fund Status" ko="한활 후원과 자금 현황" />
          </h2>
          <p className="mt-5 text-sm leading-7 text-ink/68">
            <I18nText
              en="Donations support Hanhwal activities, K-culture projects, international clubs, and community programs."
              ko="후원금은 Hanhwal 활동, K-컬처 프로젝트, 국제 학생 클럽, 커뮤니티 프로그램 운영 준비에 사용됩니다."
            />
          </p>
      ...
      {isSuperAdmin ? (
        <section className="paper-panel p-6 md:p-8">
          ...
          <p className="mt-4 text-sm leading-7 text-ink/64">
            <I18nText
              en="This admin area appears only when you are logged in as the super admin on the same Hanhwal fund page. Total donations and remaining balance can be entered manually here."
              ko="같은 Hanhwal 자금관리 주소에서 슈퍼관리자로 로그인한 경우에만 보이는 관리 영역입니다. 후원금 총액과 계좌 잔액은 여기서 직접 입력해 표시할 수 있습니다."
            />
          </p>
