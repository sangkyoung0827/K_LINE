...
const initialForm = {
  currentStatus: "previous HANHWAL member",
  departmentOrMajor: "",
  fullName: "",
  kakaoDisplayName: "",
  kakaoId: "",
...
export function HanhwalRejoinRequestForm() {
  ...
  useEffect(() => {
    void fetch("/api/hanhwal-alumni/rejoin-requests")
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/hanhwal-alumni/rejoin-requests", {
        method: "POST",
    ...
    return (
      <div className="paper-panel p-6">
        ...
        <p className="mt-3 text-sm leading-7 text-ink/64">
          Please log in with Google before submitting a Rejoin HANHWAL request.
        </p>
        <Link href="/login?callbackUrl=/hanhwal-alumni/rejoin" className="mt-5 inline-flex min-h-11 items-center bg-ink px-5 text-sm font-semibold text-paper">
          Log in to continue
        </Link>
      </div>
    );
  ...
  if (eligible === false) {
    return (
      <div className="paper-panel p-6">
        <h2 className="font-serif text-3xl font-semibold text-ink">New to HANHWAL?</h2>
        <p className="mt-3 text-sm leading-7 text-ink/64">
          If you are new to HANHWAL, please use New Member Registration instead.
        </p>
        <Link href="/hanhwal-join" className="mt-5 inline-flex min-h-11 items-center bg-ink px-5 text-sm font-semibold text-paper">
    ...
    <form onSubmit={submit} className="paper-panel grid gap-4 p-5 md:p-8">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <select className="form-field md:col-span-2" value={form.currentStatus} onChange={(event) => update("currentStatus", event.target.value)}>
          ...
          <option>previous HANHWAL member</option>
          <option>other</option>
        </select>
