...
import { Bot, Compass, Map, MapPinned, Sparkles, UserRound } from "lucide-react";
...
const routes = [
  ...
  { href: "/jeju/map", icon: Map, label: "My Map" },
  { href: "/jeju/discover", icon: MapPinned, label: "Discover" },
  { href: "/jeju/ai", icon: Bot, label: "Ask Woohyukmon" },
  { href: "/jeju/memories", icon: Sparkles, label: "Memories" },
  { href: "/jeju/profile", icon: UserRound, label: "Profile" }
];

export function JejuShell({ children, eyebrow, title, description, actions }: {
  children: ReactNode;
  eyebrow?: string;
  title: string;
  description?: string;
  ...
  return (
    <main className="min-h-[calc(100svh-76px)] overflow-x-hidden bg-[#eff8f5] text-ink sm:min-h-[calc(100svh-92px)]">
      ...
      <nav aria-label="Jeju Explorer" className="sticky top-[76px] z-30 border-b border-[#0d5962]/10 bg-[#f9fdfb]/95 backdrop-blur sm:top-[92px]">
        <div className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-3 py-2 sm:px-5 lg:px-8">
          ...
          <Link
            href="/jeju/program"
            className={`inline-flex shrink-0 items-center gap-2 rounded-full px-3 py-2 text-xs font-bold transition sm:px-4 sm:text-sm ${
              pathname.startsWith("/jeju/program")
                ? "bg-[#0d5962] text-white shadow-[0_8px_18px_rgba(13,89,98,0.18)]"
                : "text-[#315b5f] hover:bg-[#dcedea] hover:text-[#073c44]"
            }`}
          >
