...
export function HanhwalAlumniNoticeAdminPanel() {
  ...
  const load = async () => {
    const data = await fetch("/api/hanhwal-alumni/notices").then((response) => response.json());
  ...
  const create = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch("/api/hanhwal-alumni/notices", {
      method: "POST",
  ...
  const remove = async (id: string) => {
    const data = await fetch(`/api/hanhwal-alumni/notices?id=${encodeURIComponent(id)}`, {
      method: "DELETE"
...
export function HanhwalAlumniInquiryAdminPanel() {
  const [items, setItems] = useState<Inquiry[]>([]);

  const load = async () => {
    const data = await fetch("/api/hanhwal-alumni/inquiries").then((response) => response.json());
  ...
  const save = async (item: Inquiry, status: string) => {
    await fetch("/api/hanhwal-alumni/inquiries", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
export function HanhwalRejoinRequestAdminPanel() {
  const [items, setItems] = useState<RejoinRequest[]>([]);

  const load = async () => {
    const data = await fetch("/api/hanhwal-alumni/rejoin-requests").then((response) => response.json());
  ...
  const save = async (item: RejoinRequest, status: string) => {
    await fetch("/api/hanhwal-alumni/rejoin-requests", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
