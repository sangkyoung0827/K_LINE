...
type RolesResponse = {
  developerInfo: null | {
    ...
    hanhwalOfficialTeamChatUrl: string;
...
const roleLabels: Record<HanhwalRole, { en: string; ko: string }> = {
...
export function HanhwalPermissionManagementPanel() {
    ...
    <section className="paper-panel grid gap-6 p-5 md:p-8">
      ...
      {data?.developerInfo ? (
        <div className="border border-brass/25 bg-brass/10 p-4 text-sm leading-7 text-ink">
          ...
          <p className="break-all">HANHWAL_OFFICIAL_TEAM_CHAT_URL: {data.developerInfo.hanhwalOfficialTeamChatUrl}</p>
        </div>
      ) : null}
    </section>
  );
