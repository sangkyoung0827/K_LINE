...
export function TraditionalLiquorRealImportPanel() {
  ...
  return <section className="mt-6 border border-white/10 bg-white/[0.025]">
    ...
    <header className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 p-5">
      <div><div className="flex items-center gap-2 text-[11px] font-bold tracking-[0.14em] text-[#f7c76b]"><Upload className="h-4 w-4" />BUSINESS DATA FILE IMPORT</div><h2 className="mt-2 text-xl font-semibold">비즈니스 데이터 수집 · 파일 가져오기</h2><p className="mt-1 text-xs leading-5 text-white/42">CSV · XLSX · JSON (최대 4MB / 10,000행) → Staging → Entity Resolution → 관리자 승인 → Production</p><p className="mt-1 text-[10px] leading-4 text-white/32">이 화면은 영구 전통주 DB 등록용입니다. 우혁몬 보고서용 일회성 수집분은 자동 분석 후 저장하지 않습니다.</p></div>
