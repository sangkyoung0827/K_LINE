...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(await runBoundedFinanceJob(() => financeEngine.createPaperTradeProposal(symbol)), { status: 202 });
  } catch (error) {
    ...
    return NextResponse.json({ error: "Paper trade proposal is temporarily unavailable." }, { status: 503 });
  }
}
