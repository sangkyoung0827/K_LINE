...
function errorResponse(error: unknown) {
  if (error instanceof SupabaseConfigError) {
    return NextResponse.json({ error: "HANHWAL fund storage is not configured." }, { status: 503 });
  }
  if (error instanceof SupabaseRequestError && error.status === 404) {
    return NextResponse.json({ error: "HANHWAL fund table is not ready." }, { status: 503 });
  }
  console.error("HANHWAL fund API error", error);
  return NextResponse.json({ error: "HANHWAL fund storage is temporarily unavailable." }, { status: 500 });
}
...
export async function GET() {
  try {
    ...
    const rows = await supabaseRequest<FundRow[]>(`hanhwal_fund_settings?select=${columns}&id=eq.hanhwal&limit=1`);
...
export async function PATCH(request: Request) {
  try {
    ...
    const rows = await supabaseRequest<FundRow[]>(`hanhwal_fund_settings?id=eq.hanhwal&select=${columns}`, {
      method: "PATCH",
