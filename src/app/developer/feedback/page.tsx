...
export const metadata: Metadata = createNoIndexMetadata({
  title: "Developer Site Feedback",
  description: "K_LINE developer-only site feedback review page.",
...
export default async function DeveloperFeedbackPage() {
  ...
  if (!session?.user) {
    return (
      <DeveloperFeedbackGate
        icon={<LockKeyhole aria-hidden className="h-12 w-12 text-brass" />}
        title="Login required"
        description="Google login is required before opening the developer feedback page."
        ctaHref="/login"
  ...
  if (!access.isDeveloper) {
    return (
      <DeveloperFeedbackGate
        ...
        description="Site feedback is available only to developer accounts."
        ctaHref="/our-activities/ecc"
        ctaLabel="Back to ECC"
      />
    );
  }

  return <DeveloperFeedbackPanel />;
}

function DeveloperFeedbackGate({
  ctaHref,
  ctaLabel,
  description,
  icon,
