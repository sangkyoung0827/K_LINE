...
export type HanhwalMemberRegistrationStatus =
  | "submitted"
  | "payment_pending"
  | "approved"
  | "rejected";

export type HanhwalMemberRegistrationRow = {
  id: string;
  ...
  status: HanhwalMemberRegistrationStatus | null;
  admin_note: string | null;
  created_at: string;
  updated_at: string | null;
};

export type HanhwalMemberRegistration = {
  id: string;
  siteMemberId: string;
  googleEmail: string;
  googleName: string;
  ...
  status: HanhwalMemberRegistrationStatus;
  adminNote: string;
  createdAt: string;
  updatedAt: string;
};

export type HanhwalMemberRegistrationFormInput = {
...
export const hanhwalMemberRegistrationsTable = "hanhwal_member_registrations";
export const hanhwalMemberRegistrationColumns =
...
export function toHanhwalMemberRegistration(row: HanhwalMemberRegistrationRow): HanhwalMemberRegistration {
  return {
...
export function cleanHanhwalMemberRegistrationFormInput(input: HanhwalMemberRegistrationFormInput) {
  return {
...
export function validateHanhwalMemberRegistrationForm(input: ReturnType<typeof cleanHanhwalMemberRegistrationFormInput>) {
...
export async function getHanhwalMemberRegistrationByEmail(email?: string | null) {
  ...
  const rows = await supabaseRequest<HanhwalMemberRegistrationRow[]>(
    `${hanhwalMemberRegistrationsTable}?select=${hanhwalMemberRegistrationColumns}&google_email=eq.${encodeURIComponent(
      normalized
...
export async function listHanhwalMemberRegistrations() {
  ...
  return rows.map(toHanhwalMemberRegistration);
}
  ...
  if (existing) {
    const rows = await supabaseRequest<HanhwalMemberRegistrationRow[]>(
      `${hanhwalMemberRegistrationsTable}?id=eq.${encodeURIComponent(
        existing.id
      )}&select=${hanhwalMemberRegistrationColumns}`,
      {
        method: "PATCH",
        headers: {
    ...
    return rows[0] ? toHanhwalMemberRegistration(rows[0]) : existing;
  }

  const rows = await supabaseRequest<HanhwalMemberRegistrationRow[]>(
    `${hanhwalMemberRegistrationsTable}?select=${hanhwalMemberRegistrationColumns}`,
    {
      method: "POST",
      headers: {
        Prefer: "return=representation"
