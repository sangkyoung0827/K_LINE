...
import { getCurrentHanhwalAccess, getHanhwalRoleRow } from "@/lib/hanhwalAccess";
import { getHanhwalMemberRegistrationByEmail } from "@/lib/hanhwalMemberRegistrations";
...
export function canReadNotice(
  ...
  access: Awaited<ReturnType<typeof getCurrentHanhwalAccess>>,
...
export async function hasPreviousHanhwalHistory(email?: string | null) {
  ...
  const [registration, roleRow] = await Promise.all([
    getHanhwalMemberRegistrationByEmail(normalized).catch(() => null),
    getHanhwalRoleRow(normalized).catch(() => null)
  ]);
...
export async function listReadableAlumniNotices() {
  ...
  const hasHistory = await hasPreviousHanhwalHistory(access.email);
  const rows = await supabaseRequest<AlumniNoticeRow[]>(
    `hanhwal_alumni_notices?select=${alumniNoticeColumns}&order=is_pinned.desc,created_at.desc&limit=100`
  );
...
export async function listAllAlumniNotices() {
  return supabaseRequest<AlumniNoticeRow[]>(
    `hanhwal_alumni_notices?select=${alumniNoticeColumns}&order=is_pinned.desc,created_at.desc&limit=500`
  );
}
...
export async function createAlumniNotice(input: {
  ...
  const rows = await supabaseRequest<AlumniNoticeRow[]>(
    `hanhwal_alumni_notices?select=${alumniNoticeColumns}`,
    {
      method: "POST",
...
export async function patchAlumniNotice(input: {
  ...
  const rows = await supabaseRequest<AlumniNoticeRow[]>(
    `hanhwal_alumni_notices?id=eq.${encodeURIComponent(input.id)}&select=${alumniNoticeColumns}`,
    {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
...
export async function deleteAlumniNotice(id: string) {
  await supabaseRequest<null>(`hanhwal_alumni_notices?id=eq.${encodeURIComponent(id)}`, {
    method: "DELETE",
...
export async function listAlumniInquiries(admin = false, email = "") {
  const query = admin
    ? `hanhwal_alumni_activity_inquiries?select=${alumniInquiryColumns}&order=created_at.desc&limit=500`
    : `hanhwal_alumni_activity_inquiries?select=${alumniInquiryColumns}&email=eq.${encodeURIComponent(
...
export async function createAlumniInquiry(input: Record<string, unknown>, email: string) {
  ...
  const rows = await supabaseRequest<AlumniInquiryRow[]>(
    `hanhwal_alumni_activity_inquiries?select=${alumniInquiryColumns}`,
    {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify({
...
export async function patchAlumniInquiry(input: {
  ...
  const now = new Date().toISOString();
  const rows = await supabaseRequest<AlumniInquiryRow[]>(
    `hanhwal_alumni_activity_inquiries?id=eq.${encodeURIComponent(
      input.id
...
export async function listRejoinRequests(admin = false, email = "") {
  const query = admin
    ? `hanhwal_rejoin_requests?select=${rejoinRequestColumns}&order=created_at.desc&limit=500`
    : `hanhwal_rejoin_requests?select=${rejoinRequestColumns}&google_email=eq.${encodeURIComponent(
        normalizeEmail(email)
      )}&order=created_at.desc&limit=50`;
...
export async function createRejoinRequest(input: Record<string, unknown>) {
  ...
  const rows = await supabaseRequest<RejoinRequestRow[]>(
    `hanhwal_rejoin_requests?select=${rejoinRequestColumns}`,
    {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify({
...
export async function patchRejoinRequest(input: {
  ...
  const now = new Date().toISOString();
  const rows = await supabaseRequest<RejoinRequestRow[]>(
    `hanhwal_rejoin_requests?id=eq.${encodeURIComponent(input.id)}&select=${rejoinRequestColumns}`,
    {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify({
